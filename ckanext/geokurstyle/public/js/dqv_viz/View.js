class View {
    ...
    translate_nodes(x, y, coords_old) {
        ...
        let dist_x = this.sketch.mouseX - x;
        let dist_y = this.sketch.mouseY - y;
    ...
    zoom_in() {
        ...
        for (let p5node of Object.values(this.p5nodes)) {
            let dist_x = this.sketch.mouseX - p5node.get_x();
            dist_x *= scale_factor;
            let x_new = this.sketch.mouseX - dist_x;
            p5node.set_x(x_new);
            p5node.target_x = null;

            let dist_y = this.sketch.mouseY - p5node.get_y();
            dist_y *= scale_factor;
            let y_new = this.sketch.mouseY - dist_y;
            p5node.set_y(y_new);
            p5node.target_y = null;
    ...
    update_edges(edges) {
        ...
        for (let edge of edges) {
            this.p5edges.push(new P5ArrowConnector(this.p5nodes[edge.from], this.p5nodes[edge.to], edge.label, this.sketch))
        }
    }
